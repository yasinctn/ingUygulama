...
final class KelimeSecScreen extends StatefulWidget {
  const KelimeSecScreen({super.key});

  @override
  State<KelimeSecScreen> createState() => _KelimeSecScreenState();
}

final class _KelimeSecScreenState extends State<KelimeSecScreen> {
// DEĞİŞKENLER
  List<Kelime> words = Fruits.fruits;
...
// YANLIŞ SEÇENEKLERİ OLUŞTURMA FONKSİYONU
  Kelime generateUncorrectWord(Kelime correctAnsver) {
    List<Kelime> otherWords = List.from(Fruits.fruits)..remove(correctAnsver);
...
// SORU OLUŞTURMA FONKSİYONU
  void generateQuestion() {
    ...
    if (correctWord != null) {
      imagePath = correctWord!.imagePath;
  ...
  void prevWord() {
    if (index > 0) {
      ...
      // Optionally handle when there are no previous questions
...
// CEVAP KONTROL FONKSİYONU
  bool checkAnswer(Kelime selectedWord) {
    if (selectedWord.ing == correctWord!.ing) {
      return true;
    } else {
      return false;
    }
